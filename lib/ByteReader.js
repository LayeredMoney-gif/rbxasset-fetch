...
class ByteReader extends Uint8Array {
	...
	Zstd(comLength, decomLength, output) {
		if (this.GetRemaining() < comLength) throw new Error("[ByteReader.Zstd] unexpected eof")
	...
	LZ4(comLength, decomLength, output) {
		if (this.GetRemaining() < comLength) throw new Error("[ByteReader.LZ4] unexpected eof")
		...
		while (true) {
			...
			if (this.index + literalLength > endIndex) throw new Error("[ByteReader.LZ4] unexpected eof")
			...
			if (outputIndex + matchLength > decomLength) throw new Error("[ByteReader.LZ4] output size mismatch")
		...
		if (this.index !== endIndex) throw new Error("[ByteReader.LZ4] input size mismatch")
		if (outputIndex !== decomLength) throw new Error("[ByteReader.LZ4] output size mismatch")
...
export { ByteReader, bufferToString }
